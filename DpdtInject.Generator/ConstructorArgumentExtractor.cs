...
namespace DpdtInject.Generator
{
    public class ConstructorArgumentExtractor : CSharpSyntaxRewriter
    {
        ...
        public override SyntaxNode VisitObjectCreationExpression(Microsoft.CodeAnalysis.CSharp.Syntax.ObjectCreationExpressionSyntax node)
        {
    ...
    public class DetectedConstructorArgument
    {
            ...
            )
        {
            ...
            if (this.DefineInBindNode)
            {
                return $"{Name}: {Body}";
            }

            //we need to access child instance container
            var childBindContainers = bindingProcessorContainer.GetBindWith(this.Type!.GetFullName());
